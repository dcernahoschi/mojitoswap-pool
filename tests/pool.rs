...
impl Context {
    ...
    pub fn new(fee: Decimal, sqrt_price: Decimal) -> Self {
        ...
        let owner_badge_addr: ResourceAddress = result.new_resource_addresses()[2];
        ...
        let new_pool_manif = ManifestBuilder::new()
            .call_function(
                ...
                "Pool",
                "new",
                manifest_args![moj_addr, usdt_addr, fee, sqrt_price, owner_badge_addr],
            )
            .call_method(
                admin.addr,
                "deposit_batch",
    ...
    fn transfer_owner_badge_from_admin_to_account(&mut self, account: &Account) {
        let owner_transfer_manif =
            make_owner_transfer_manifest(&self.admin, self.owner_badge_addr, account);
        execute_manif(
            &mut self.runner,
            owner_transfer_manif,
            vec![&self.admin.pub_key],
        );
    }

    fn transfer_owner_badge_from_account_to_admin(&mut self, account: &Account) {
        let owner_transfer_manif =
            make_owner_transfer_manifest(account, self.owner_badge_addr, &self.admin);
        execute_manif(
            &mut self.runner,
            owner_transfer_manif,
            vec![&account.pub_key],
    ...
    pub fn add_position(
        ...
        self.transfer_owner_badge_from_admin_to_account(account);
            ...
            .take_from_worktop(self.moj_addr, |builder1, moj_bucket| {
                builder1
                    ...
                    .take_from_worktop(self.usdt_addr, |builder2, usdt_bucket| {
                        builder2
                            .create_proof_from_account(account.addr, self.owner_badge_addr)
                            .create_proof_from_auth_zone(
                                self.owner_badge_addr,
                                |builder3, owner_proof| {
        ...
        self.transfer_owner_badge_from_account_to_admin(account);
                            ...
                            .create_proof_from_auth_zone(
                                ...
                                |builder3, proof| {
                                    builder3
                                        ...
                                        .create_proof_from_auth_zone(
                                            self.owner_badge_addr,
                                            |builder4, owner_proof| {
                                                builder4.call_method(
                                                    self.pool_addr,
                                                    "add_liq",
                                                    manifest_args![
                                                        moj_bucket,
    ...
    pub fn add_accumulated_fees_to_liq(&mut self, account: &Account) -> TransactionReceipt {
        ...
        let add_liq_manif = ManifestBuilder::new()
            ...
            .create_proof_from_auth_zone(self.position_nft_addr, |builder, proof| {
                builder
                    .create_proof_from_account(account.addr, self.owner_badge_addr)
                    .create_proof_from_auth_zone(self.owner_badge_addr, |builder1, owner_proof| {
                        builder1.call_method(
                            self.pool_addr,
                            "add_accumulated_fees_to_liq",
                            manifest_args![proof, owner_proof],
                        )
                    })
            })
            .build();
    ...
    pub fn remove_pos(
        ...
        let remove_liq_manif = ManifestBuilder::new()
            ...
            .create_proof_from_auth_zone(self.position_nft_addr, |builder1, proof| {
                builder1
                    .create_proof_from_account(account.addr, self.owner_badge_addr)
                    .create_proof_from_auth_zone(self.owner_badge_addr, |builder2, owner_proof| {
                ...
                builder1
                    ...
                    .create_proof_from_auth_zone(self.owner_badge_addr, |builder2, owner_proof| {
                        builder2.call_method(
                            self.pool_addr,
                            "swap",
                            manifest_args![token_bucket, owner_proof],
                        )
                    })
            })
...
/**
 * Transfers a resource between the given accounts
 */
fn make_owner_transfer_manifest(
    owner: &Account,
    resource_addr: ResourceAddress,
    account: &Account,
) -> TransactionManifest {
    ManifestBuilder::new()
        .withdraw_from_account(owner.addr, resource_addr, Decimal::one())
        .call_method(
            account.addr,
            "deposit_batch",
            manifest_args!(ManifestExpression::EntireWorktop),
...
fn add_position() {
    ...
    if let TransactionResult::Commit(result) = &add_pos_receipt.result {
        ...
        assert_eq!(
            ...
            Some(&String::from("9999.999999999999969789"))
        );
...
fn scenario_4() {
    ...
    context.add_position(&account, dec!("10000"), dec!("10000"), -1000, 1000);
    let _swap1_receipt =
        context.swap_moj_for_usdt(&account, dec!("10000"), dec!("9534.995457587771399551"));
    let _swap2_receipt = context.swap_usdt_for_moj(
        &account,
        dec!("9534.995457587771399551"),
        dec!("9999.999999999999794948"),
    );
}
...
fn scenario_9() {
    ...
    context.swap_usdt_for_moj(
        &account,
        dec!("6470.845461125638454765"),
        dec!("4902.04694616053098079"),
    );
    let _add_liq_receipt = context.add_accumulated_fees_to_liq(&account);
    let _acc_fees_receipt =
        context.collect_fees(&account, dec!("1.446904508884543512"), Decimal::zero());
    context.remove_pos(
        &account,
        dec!("7504.799472306593846201"),
        dec!("9999.999999999847688331"),
    );
}
...
fn scenario_10() {
    ...
    let _acc1_fees_receipt =
        context.collect_fees(&account1, dec!("49.999999999999869792"), Decimal::zero());
